...
/// parsing slower.
AudioMetadata readMetadata(String path, {bool getImage = false}) {
...
String? readLyrics(String path) {
  ...
  try {
    if (ID3v2Parser.canUserParser(reader)) {
      final mp3Metadata = ID3v2Parser().parse(reader) as Mp3Metadata;

      return mp3Metadata.lyrics;
    } else if (FlacParser.canUserParser(reader)) {
      final vorbisMetadata = FlacParser().parse(reader) as VorbisMetadata;

      return vorbisMetadata.lyrics;
    } else if (MP4Parser.canUserParser(reader)) {
      final mp4Metadata = MP4Parser().parse(reader) as Mp4Metadata;

      return mp4Metadata.lyrics;
    } else if (OGGParser.canUserParser(reader)) {
      final oggMetadata = OGGParser().parse(reader) as VorbisMetadata;

      return oggMetadata.lyrics;
    } else if (RiffParser.canUserParser(reader)) {
      return null;
    } else if (ID3v1Parser.canUserParser(reader)) {
      final mp3Metadata = ID3v1Parser().parse(reader) as Mp3Metadata;
...
ParserTag readAllMetadata(String path, {bool getImage = true}) {
  final file = File(path);
  ...
  try {
    ...
    print(trace);
    throw MetadataParserException(track: file, message: e.toString());
  }
