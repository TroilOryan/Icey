...
import '../core/lyric_model.dart';
import '../core/lyric_style.dart';
import '../render/lyric_layout.dart';
import '../widgets/mixins/lyric_line_switch_mixin.dart';
...
class LyricPainter extends CustomPainter {
  ...
  void drawLine(
    ...
    // 计算模糊效果
    double calculateBlur() {
      if (!blur || isActive) return 0;

      // 计算距离当前行的索引距离
      final distance = (index - playIndex).abs();

      // 模糊参数配置
      const maxBlurSigma = 6.0; // 最大模糊度（sigma值）
      const maxDistance = 8; // 模糊达到最大值的距离（行数）

      // 根据距离计算模糊程度（线性渐变，可改为非线性）
      final blurSigma = (distance / maxDistance * maxBlurSigma).clamp(0.0, maxBlurSigma);

      return blurSigma;
    }

    final blurSigma = calculateBlur();
    ...
    painter.text = TextSpan(
      ...
      style: replaceTextStyle(oldSpan.style!, layout.style.selectedColor),
    );
    ...
    if (isActive) {
      ...
      drawHighlight(canvas, size, metric.metrics, highlightTotalWidth: double.infinity);
    }
